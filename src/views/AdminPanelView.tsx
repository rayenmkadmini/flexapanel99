...
export const AdminPanelView: React.FC = () => {
  ...
  const [dbProvider, setDbProvider] = useState(settings.database?.provider || 'local');
  ...
  const [dbEnabled, setDbEnabled] = useState(settings.database?.isEnabled || false);
        ...
        <div className="space-y-6 max-w-4xl mx-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">أفضل خيار مباشر للواجهة الحالية هو Supabase Postgres. باقي الخيارات مثل Redis و MongoDB تحتاج Backend API آمن.</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                ['supabase', 'Supabase', 'جاهز للربط الآن'],
                ['mongodb', 'MongoDB Atlas', 'يتطلب Backend'],
                ['redis', 'Redis', 'يتطلب Backend'],
                ['turso', 'Turso', 'يتطلب Backend'],
                ['convex', 'Convex', 'يتطلب SDK منفصل'],
                ['prisma', 'Prisma Postgres', 'يتطلب Backend']
              ].map(([value, label, hint]) => (
                <button
                  key={value}
                  onClick={() => setDbProvider(value as any)}
                  className={`p-4 rounded-2xl border text-right transition-all ${dbProvider === value ? 'bg-blue-600/20 border-blue-500 text-white' : 'bg-slate-800/60 border-slate-700 text-slate-300 hover:bg-slate-800'}`}
                >
            ...
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between gap-3">
                ...
                <label className="flex items-center gap-2 text-xs font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-3 py-2 cursor-pointer">
                  <input type="checkbox" checked={dbEnabled} onChange={e => setDbEnabled(e.target.checked)} className="accent-emerald-500" />
                  <span>تفعيل</span>
                </label>
              </div>
            ...
            <div>
              <label className="block text-sm font-bold text-slate-300 mb-1">بريد حساب PayPal</label>
                <input
                  ...
                  placeholder="payments@flexapanel.net"
                  className="w-full bg-slate-800 border border-slate-700 rounded-2xl px-4 py-3.5 text-white font-mono text-base focus:outline-none focus:border-blue-500 text-left"
                  dir="ltr"
                  required
                />
              ...
              {newSrvIsDigital && (
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="email1@flexapanel.net:pass123&#10;email2@flexapanel.net:pass456"
