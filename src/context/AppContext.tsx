...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => getSaved('currentUser', INITIAL_USERS[2])); // Default to ahmed_vip
  ...
  const syncProviderServices = async (providerId: string): Promise<boolean> => {
    ...
    try {
      ...
      const res = await fetch(prov.apiUrl, {
        ...
        body: JSON.stringify({ key: prov.apiKey, action: 'services' })
      });
  ...
  const syncProviderBalance = async (providerId: string): Promise<boolean> => {
    ...
    try {
      const res = await fetch(prov.apiUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ key: prov.apiKey, action: 'balance' })
      });
      const data = await res.json();
